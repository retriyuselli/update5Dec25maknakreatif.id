        ...
        <div class="space-y-6">
            <div class="bg-white shadow rounded-lg border border-gray-200 p-4">
                <form method="GET" class="flex flex-wrap items-center gap-2">
                    <input type="date" name="filter_start" value="{{ request()->query('filter_start', $record->period_start->format('Y-m-d')) }}" class="border border-gray-300 rounded px-2 py-1 text-sm" />
                    <input type="date" name="filter_end" value="{{ request()->query('filter_end', $record->period_end->format('Y-m-d')) }}" class="border border-gray-300 rounded px-2 py-1 text-sm" />
                    <select name="filter_type" class="border border-gray-300 rounded px-2 py-1 text-sm">
                    ...
                    @php $gp = (int) request()->query('per_page', 50); @endphp
                    ...
                    <button type="submit" class="bg-gray-800 hover:bg-black text-white px-3 py-1 rounded text-sm">Terapkan</button>
                    <a href="{{ request()->url() }}" class="text-gray-600 hover:text-gray-800 text-sm">Reset</a>
                </form>
            </div>
            @php
                $matchRateApp = $statistics['total_app_transactions'] > 0 
                    ? round(($statistics['matched_count'] / $statistics['total_app_transactions']) * 100, 1) 
                    : 0;
                $matchRateBank = $statistics['total_bank_items'] > 0 
                    ? round(($statistics['matched_count'] / $statistics['total_bank_items']) * 100, 1) 
                    : 0;
                $globalPerPage = (int) request()->query('per_page', 50);
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm">
                            ...
                            <tbody class="divide-y divide-gray-200">
                                @php $runningBalance = (float) ($record->opening_balance ?? 0); @endphp
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div class="bg-white p-6 rounded-lg shadow border border-gray-200">
                    <div class="text-center">
                        <div class="text-3xl font-bold text-blue-600">App {{ $matchRateApp }}% • Bank {{ $matchRateBank }}%</div>
                        <div class="text-sm text-gray-500 mt-1">Tingkat Kecocokan</div>
                        <div class="text-xs text-gray-400 mt-2">Per App • Per Bank</div>
                    </div>
                </div>
            </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-sm">
                                ...
                                <tfoot class="bg-gray-50 border-t border-gray-200">
                                    <tr>
                                        <td class="px-6 py-3 text-left text-sm text-gray-600" colspan="6">
                                            Menampilkan 
                                            {{ min(($matchedPage - 1) * $matchedPerPage + 1, $matchedTotal) }}–
                                            {{ min($matchedPage * $matchedPerPage, $matchedTotal) }} 
                                            dari {{ $matchedTotal }}
                                            <span class="ml-4">
                                                @if($matchedPage > 1)
                                                    <a class="text-blue-600 hover:text-blue-800" href="{{ request()->fullUrlWithQuery(['matched_page' => $matchedPage - 1, 'matched_per_page' => $matchedPerPage]) }}">Prev</a>
                                                @else
                                                    <span class="text-gray-400">Prev</span>
                                                @endif
                                                <span class="mx-2">|</span>
                                                @if($matchedPage < $matchedTotalPages)
                                                    <a class="text-blue-600 hover:text-blue-800" href="{{ request()->fullUrlWithQuery(['matched_page' => $matchedPage + 1, 'matched_per_page' => $matchedPerPage]) }}">Next</a>
                                                @else
                            ...
                            <table class="w-full text-sm">
                                ...
                                <tbody class="divide-y divide-gray-200">
                                    @php
                                        ...
                                        if ($ua instanceof \Illuminate\Support\Collection) {
                                            $ua = $ua->all();
                                        }
                                        if ($fsC || $feC || $filterType !== 'all') {
                                        ...
                                        <tr>
                                            ...
                                            <td class="px-6 py-3" colspan="2">
                                                <div class="text-sm text-gray-600">
                                                    Menampilkan 
                                                    {{ min(($uaPage - 1) * $uaPerPage + 1, $uaTotal) }}–
                                                    {{ min($uaPage * $uaPerPage, $uaTotal) }} 
                                                    dari {{ $uaTotal }}
                                                    <span class="ml-4">
                                                        @if($uaPage > 1)
                                                            <a class="text-blue-600 hover:text-blue-800" href="{{ request()->fullUrlWithQuery(['ua_page' => $uaPage - 1, 'ua_per_page' => $uaPerPage]) }}">Prev</a>
                                                        @else
                                                            <span class="text-gray-400">Prev</span>
                                                        @endif
                                                        <span class="mx-2">|</span>
                                                        @if($uaPage < $uaTotalPages)
                                                            <a class="text-blue-600 hover:text-blue-800" href="{{ request()->fullUrlWithQuery(['ua_page' => $uaPage + 1, 'ua_per_page' => $uaPerPage]) }}">Next</a>
                                                        @else
                                                            <span class="text-gray-400">Next</span>
                                                        @endif
                                                    </span>
                            ...
                            <table class="w-full text-sm">
                                ...
                                <tbody class="divide-y divide-gray-200">
                                    @php
                                        ...
                                        if ($ub instanceof \Illuminate\Support\Collection) {
                                            $ub = $ub->all();
                                        }
                                        if ($fsC || $feC || $filterType !== 'all') {
                                        ...
                                        $ubTotal = count($ub);
                                        $ubPerPage = (int) request()->query('ub_per_page', $globalPerPage);
                                    ...
                                    <tfoot class="bg-gray-50 border-t border-gray-200">
                                        <tr>
                                            ...
                                            <td class="px-6 py-3">
                                                <div class="text-sm text-gray-600">
                                                    Menampilkan 
                                                    {{ min(($ubPage - 1) * $ubPerPage + 1, $ubTotal) }}–
                                                    {{ min($ubPage * $ubPerPage, $ubTotal) }} 
                                                    dari {{ $ubTotal }}
                                                    <span class="ml-4">
                                                        @if($ubPage > 1)
                                                            <a class="text-blue-600 hover:text-blue-800" href="{{ request()->fullUrlWithQuery(['ub_page' => $ubPage - 1, 'ub_per_page' => $ubPerPage]) }}">Prev</a>
                                                        @else
                                                            <span class="text-gray-400">Prev</span>
                                                        @endif
                                                        <span class="mx-2">|</span>
                                                        @if($ubPage < $ubTotalPages)
                                                            <a class="text-blue-600 hover:text-blue-800" href="{{ request()->fullUrlWithQuery(['ub_page' => $ubPage + 1, 'ub_per_page' => $ubPerPage]) }}">Next</a>
                                                        @else
                                                            <span class="text-gray-400">Next</span>
                                                        @endif
                                                    </span>
